...
type handleFType = (req: MessageReq, sender: chrome.runtime.MessageSender, sendResponse: (res: MessageRes | Promise<MessageRes>) => void, next?: () => void) => void;
...
chrome.runtime.onMessage.addListener((req: MessageReq, sender, sendResponse) => {
    ...
    if (handleListeners.length <= 0) { return false; }
    const handleF = () => {
        if (handleListeners.length <= 0) { return; }
        let onHandle = handleListeners.shift()!;
        onHandle.handleF(req, sender, (res) => {
